 ...
 */
async function startLogSession(description: string, options: { project?: string, tags?: string }) {
 ...
 */
async function endLogSession(options: { summary?: boolean }) {
 ...
 */
async function addLogEntry(content: string, options: { tags?: string, type?: string }) {
...
async function showLogs(options: { 
  ...
  try {
    ...
    if (options.recent !== undefined) {
      ...
      if (options.json) {
        ...
        for (const session of recentSessions) {
          ...
          // Show entry counts
          if (session.entries) {
            const entryCount = session.entries.filter(e => 
              e.type !== LogEntryType.SESSION_START && 
              e.type !== LogEntryType.SESSION_END
            ).length;
    ...
    logger.error('Error loading logs', error as Error);
  }
}
 ...
 */
async function searchLogEntries(term: string, options: {
  limit?: number,
  project?: string,
  startDate?: string,
  endDate?: string
  ...
  try {
    ...
    logger.error('Error searching logs', error as Error);
  }
}
