...
(function (LogEntryType) {
    LogEntryType["SESSION_START"] = "session_start";
    LogEntryType["SESSION_END"] = "session_end";
    LogEntryType["NOTE"] = "note";
    LogEntryType["CHALLENGE"] = "challenge";
    LogEntryType["SOLUTION"] = "solution";
    LogEntryType["DECISION"] = "decision";
    LogEntryType["DISCOVERY"] = "discovery";
 ...
 */
export function getDailyLogsPath() {
 ...
 */
export function getProjectLogsPath(project) {
 ...
 */
export function createLogEntry(type, content, tags, metadata) {
    return {
        id: uuidv4(),
 ...
 */
export function getDailyLogPath(date = getCurrentDate()) {
 ...
 */
export async function startSession(description, project, tags) {
    ...
    // Add start entry
    const startEntry = createLogEntry(LogEntryType.SESSION_START, description, tags, { project });
...
export async function endSession(summary) {
    ...
    if (summary) {
        ...
        const summaryEntry = createLogEntry(LogEntryType.SESSION_END, summary, session.tags);
        session.entries.push(summaryEntry);
    }
    else {
        // Add a simple end entry
        const endEntry = createLogEntry(LogEntryType.SESSION_END, 'Session ended', session.tags);
 ...
 */
export async function addEntryToCurrentSession(type, content, tags) {
    // Get current session ID
    const sessionId = await getCurrentSessionId();
    if (!sessionId) {
 ...
 */
export async function addSessionToDailyLog(session) {
 ...
 */
export async function updateSessionInDailyLog(session) {
 ...
 */
export async function addSessionToProjectLog(session, project) {
 ...
 */
export async function updateSessionInProjectLog(session, project) {
 ...
 */
export function parseSessionFromMarkdown(markdown, sessionId) {
 ...
 */
export async function searchLogs(term, options = {}) {
 ...
 */
export async function getRecentSessions(limit = 5, project) {
    // Get all session files
    let sessionsDir = getSessionsPath();
 ...
 */
export async function getProjectsWithLogs() {
 ...
 */
export function gatherSessionContext() {
    const context = {
        os: os.platform(),
